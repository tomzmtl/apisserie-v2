...
import { Drawer, Paper, TextField, Stack, Button } from "@mui/material"
...
const Zone = ({ onAfterSave, zoneId, isOpen, add = null }) => {
  ...
  const existingZoneNames = difference(zones.map(z => z.name), isCreateMode ? [] : [zone.name]);
      ...
      <Paper square sx={{ p: 2 }}>
        <form onSubmit={handleSubmit}>
          ...
          <Stack
            ...
            sx={{ mt: 3 }}
          >
            {zId && <Button {...deleteBtnProps}>Supprimer</Button>}
